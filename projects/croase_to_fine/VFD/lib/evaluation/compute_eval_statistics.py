...
def calMAE(pre_res,tmp):
    ...
    each_prediction = {}
    cla = os.listdir('/data1/zaiyihu/Datasets/Nutrition_data/VFD/data/vfdl/test_resize')
    for i in cla :
        sum_one = 0
        cnt = 0
        this_id = os.path.join('/data1/zaiyihu/Datasets/Nutrition_data/VFD/data/vfdl/test_resize',i)

        true_id,trash = this_id.split('.')
        true_id = true_id.split('test_resize/')[1]
